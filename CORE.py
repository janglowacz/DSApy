...
layout_tab_talents =    [[sg.Frame(key='F_die_rolling', layout=layout_tab_talents_2A, element_justification='c', vertical_alignment='c', title='Die rolling', title_location='n', font=const.FL),
...
#layout_tabs = [[sg.Tab(title='Fertigkeiten', key='Tab_abilities', layout=layout_tab_talents, element_justification='c')]]
...
layout =    [[sg.Input(key='I_Char_select', visible=False, enable_events=True),
            ...
            [sg.Column(layout=layout_tab_talents)]]
...
window.finalize()
window['T_Char_attributes'].expand(expand_x=True)
window['F_die_rolling'].expand(expand_x=True, expand_y=True)
window.refresh()
...
while const.FLAG_RUN:
    ...
    # ==================================================================== EVENT
        ...
        for i in range(3):
            ...
            window['T_talents_DR'+str(i)].update(str(Results[i]))
